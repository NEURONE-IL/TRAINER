...
import {
  ApiTriviaService,
  TriviaStudy,
} from '../../services/apiTrivia/apiTrivia.service';
...
export class FlowDisplayComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.flowService
      .getFlow(this.route.snapshot.paramMap.get('flow_id'))
      .subscribe(
        (response) => {
          ...
          this.emailFormControl = new FormControl('', [
            ...
            notExistingColl(this.flow.collaborators),
          ]);
          if (this.flow.collaborators.length > 0)
            this.collaboratorsExist = true;

          if (!(this.user._id == this.flow.user._id)) this.userOwner = false;
        },
        (err) => {
          this.toastr.error(
            this.translate.instant('FLOW.TOAST.NOT_LOADED_ERROR'),
    ...
    */
    this.stageService
      .getStagesByFlowSortedByStep(this.route.snapshot.paramMap.get('flow_id'))
      .subscribe((response) => {
        this.sortedStages = response['stages'];
      });
    this.historyService
      .getHistoryByFlowByType(
        this.route.snapshot.paramMap.get('flow_id'),
        'clone'
      )
      .subscribe(
        (response) => {
  ...
  resetFlowTestUser() {
    ...
    this.flowService
      .resetFlowTestUser(this.route.snapshot.paramMap.get('flow_id'))
      .subscribe(
        (response) => {
  ...
  getTestUser() {
    this.flowService
      .getFlowTestUser(this.route.snapshot.paramMap.get('flow_id'))
      .subscribe((response) => {
        this.dummyUser = response['user'];
        // this.getProgress();
  ...
  resetTestUser() {
    this.authService.resetTestUser(this.flow._id).subscribe(
      (resp) => {
        ...
        this.toastr.success(
          this.translate.instant('FLOW.TOAST.TEST_USER_RESET_SUCCESS'),
          this.translate.instant('FLOW.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('FLOW.TOAST.TEST_USER_RESET_ERROR'),
          this.translate.instant('FLOW.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
  ...
  confirmFlowDelete(id: string) {
    confirm(this.translate.instant('ADMIN.FLOWS.DELETE_CONFIRMATION')) &&
      this.deleteFlow(id);
  }
  ...
  deleteFlow(id: string) {
    this.flowService.deleteFlow(id).subscribe(
      (flow) => {
        this.toastr.success(
          this.translate.instant('FLOW.TOAST.SUCCESS_MESSAGE_DELETE'),
          this.translate.instant('FLOW.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('FLOW.TOAST.ERROR_MESSAGE_DELETE'),
          this.translate.instant('FLOW.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  confirmStageDelete(id: string) {
    confirm(this.translate.instant('ADMIN.STAGES.DELETE_CONFIRMATION')) &&
      this.deleteStage(id);
  }
  ...
  deleteStage(id: string) {
    this.stageService.deleteStage(id).subscribe(
      (stage) => {
        this.stageService
          .getStagesByFlowSortedByStep(
            this.route.snapshot.paramMap.get('flow_id')
          )
          .subscribe((response) => (this.sortedStages = response['stages']));
        this.toastr.success(
          this.translate.instant('STAGE.TOAST.SUCCESS_MESSAGE_DELETE'),
          this.translate.instant('STAGE.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('STAGE.TOAST.ERROR_MESSAGE_DELETE'),
          this.translate.instant('STAGE.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  confirmModuleDelete(id: string) {
    confirm(this.translate.instant('ADMIN.MODULES.DELETE_CONFIRMATION')) &&
  ...
  updateStage(id: string, updatedStage: string) {
    this.stageService.putStage(id, updatedStage).subscribe(
      (stage) => {
        this.stageService
          .getStagesByFlowSortedByStep(
            this.route.snapshot.paramMap.get('flow_id')
          ...
          .subscribe((response) => (this.sortedStages = response['stages']));
        this.toastr.success(
          this.translate.instant('STAGE.TOAST.SUCCESS_MESSAGE_UPDATE') +
            stage['stage'].title,
          this.translate.instant('STAGE.TOAST.SUCCESS'),
          {
            timeOut: 5000,
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('STAGE.TOAST.ERROR_MESSAGE_UPDATE'),
          this.translate.instant('STAGE.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  showFlowUpdateDialog(): void {
    this.flowService.getFlow(this.flow._id).subscribe(
      ...
      (err) => {
        console.log(err);
        this.toastr.error(
          'Ha ocurrido un error al cargar la información del flujo',
          'Error',
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  showStageUpdateDialog(stage: Stage): void {
    this.stageService.getStage(stage._id).subscribe(
      (response) => {
        const dialogRef = this.matDialog
          .open(StageUpdateComponent, {
            width: '60%',
      ...
      (err) => {
        console.log(err);
        this.toastr.error(
          'Ha ocurrido un error al cargar la información de la etapa',
          'Error',
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  showModuleUpdateDialog(module1: Module): void {
    this.moduleService.getModule(module1._id).subscribe(
      ...
      (err) => {
        console.log(err);
        this.toastr.error(
          'Ha ocurrido un error al cargar la información del módulo',
          'Error',
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  reloadModules() {
    ...
    */
    this.moduleService
      .getModuleByFlow(this.route.snapshot.paramMap.get('flow_id'))
      .subscribe((response) => {
  ...
  deleteCollaborator(collaborator) {
    ...
    this.editCollaborator(
      newCollaboratorList,
      'Se ha eliminado correctamente el colaborador al flujo',
  ...
  addCollaborator(user: any) {
    ...
    this.editCollaborator(
      newCollaboratorList,
      'Se ha añadido correctamente el colaborador al flujo',
  ...
  editCollaborator(collaboratorList, msg1, msg2) {
    this.flowService
      .editCollaboratorsFlow(this.flow._id, collaboratorList)
      .subscribe(
        (response) => {
    ...
    this.authService.getUserbyEmail(this.emailFormControl.value).subscribe(
      ...
      (err) => {
        ...
        if (error === 'USER_NOT_CONFIRMED') {
          this.toastr.error(
            'El usuario ingresado no ha terminado su registro, debe completar el proceso confirmando en el correo recibido',
  ...
  confirmCollaborationLeft() {
    confirm(
      'Seguro que desea dejar de ser colaborador del flujo: ' + this.flow.name
  ...
  collaborationLeft() {
    ...
    this.editCollaborator(
      collaborators,
      'Ha dejado de ser colaborador del flujo: ' + this.flow.name,
  ...
  cloneFlow() {
    ...
    this.flowService.cloneFlow(this.flow._id, user_id).subscribe(
      ...
      (err) => {
        this.toastr.error(
          'El flujo seleccionado no ha podido ser clonado',
