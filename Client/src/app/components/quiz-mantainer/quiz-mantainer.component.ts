...
export class QuizMantainerComponent implements OnInit {
  ...
  loadVideos() {
    this.quizService.getVideos().subscribe((res) => {
      this.videos = [];
  ...
  confirmQuizDelete(quizId) {
    confirm(this.translate.instant('QUIZZES.TOAST.DELETE_CONFIRMATION')) &&
  ...
  deleteQuiz(quizId) {
    ...
    this.quizService.deleteQuiz(quizId).subscribe(
      (quiz) => {
        this.quizService
          .getQuizzesByUser(user._id)
          .subscribe((res) => (this.quizzes = res.quizzes));

        this.toastr.success(
          this.translate.instant('QUIZZES.TOAST.SUCCESS_MESSAGE_DELETE'),
          this.translate.instant('QUIZZES.TOAST.SUCCESS'),
          {
            timeOut: 5000,
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('QUIZZES.TOAST.ERROR_MESSAGE_DELETE'),
          this.translate.instant('QUIZZES.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
  ...
  calcularCodigoTresDigitos(numero) {
    let codigo =
      (
        (numero - ((numero - (numero % 10)) % 100) - (numero % 10)) /
        100
      ).toString() +
      (((numero - (numero % 10)) % 100) / 10).toString() +
      (numero % 10).toString();
    return codigo;
  }
  ...
  guardarPregunta(tipo) {
    ...
    let pregunta = (document.getElementById('pregunta') as HTMLInputElement)
      .value;
    let justifique;
    if (this.justificacion && this.editingQuestion < 0) {
      justifique = (document.getElementById('justifique') as HTMLInputElement)
        .value;
    }
    this.file = this.questionImage;
    let questionCode = this.calcularCodigoTresDigitos(
    ...
    let resourceCode =
      this.calcularCodigoTresDigitos(this.quizId) +
      this.calcularCodigoTresDigitos(this.ejercicio) +
      this.calcularCodigoTresDigitos(this.questions.length + 1);
  ...
  editQuestion(i) {
    ...
    if ('bonus' in this.questions[i]) {
      ...
      (document.getElementById('justificacion') as HTMLInputElement).checked =
        this.questions[i].bonus;
      this.justificacion = true;
      setTimeout(() => {
    ...
    if (
      this.questions[i].question_type == 2 ||
      this.questions[i].question_type == 3
    ) {
  ...
  cleanForm() {
    ...
    if (this.pregunta == 'multy') {
      ...
      (
        document.getElementById('alternativaCorrecta') as HTMLInputElement
      ).checked = false;
    }
  ...
  setJustifique() {
    console.log('nani?!');
    this.justificacion = (
      document.getElementById('justificacion') as HTMLInputElement
    ).checked;
  }
  ...
  addAlternative() {
    let opcion = (document.getElementById('alternativa') as HTMLInputElement)
      .value;
  ...
  crearEjercicio() {
    ...
    let descripcion = (document.getElementById('descEx') as HTMLInputElement)
      .value;
  ...
  editExercise(i) {
    ...
    setTimeout(() => {
      (document.getElementById('titleEx') as HTMLInputElement).value =
        this.ejercicios[i].title;
      (document.getElementById('fileEx') as HTMLInputElement).value = '';
      this.exerciseImage = this.ejercicios[i].resource_url;

      (document.getElementById('descEx') as HTMLInputElement).value =
        this.ejercicios[i].introduction;
      this.cleanForm;
      this.alternatives = [];
  ...
  guardarQuiz() {
    let titulo = (document.getElementById('titleQuiz') as HTMLInputElement)
      .value;
    let file = this.quizImage;
    let descripcion = (document.getElementById('descQuiz') as HTMLInputElement)
      .value;
    let id = this.quizId;
  ...
  loadImage(file) {
    ...
    this.quizService.saveImage(formData).subscribe((res) => {
      console.log(res);
    });
    return;
  }
  ...
  loadImageQuiz() {
    ...
    let formData = new FormData();
    let file = (document.getElementById('fileQuiz') as HTMLInputElement)
      .files[0];
    formData.append('file', file);
    this.quizService.saveImage(formData).subscribe((res: any) => {
