...
export class VideoMantainerComponent implements OnInit {
  ...
  registerVideo() {
    let name = (document.getElementById('tituloVideo') as HTMLInputElement)
      .value;
    let urlYoutube = (document.getElementById('urlVideo') as HTMLInputElement)
      .value;
    let url_video = '';
    let file = (document.getElementById('imageVideo') as HTMLInputElement)
      .files[0];
    let language = this.languageVideo;
    let formData = new FormData();
    if (urlYoutube && urlYoutube != '') {
      url_video = urlYoutube;
      formData.append('file', file);
      this.quizService.saveImage(formData).subscribe((res: any) => {
        ...
        this.quizService.addVideo(objetVideo).subscribe((res) => {
          console.log(res);
          this.loadVideos();
          this.cleanForm();
        });
  ...
  deleteVideo(videoId) {
    this.quizService.deleteVideo(videoId).subscribe((res) => {
      this.loadVideos();
    });
  }
